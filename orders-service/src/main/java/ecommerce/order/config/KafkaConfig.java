...
public class KafkaConfig {
  ...
  public NewTopic paymentTopic() {
    return TopicBuilder.name(PAYMENTS).partitions(3).compact().build();
  }

  @Bean
  public NewTopic stockTopic() {
    return TopicBuilder.name(STOCK).partitions(3).compact().build();
  }
  ...
  public KStream<Long, Order> stream(StreamsBuilder builder) {
    ...
    Serde<Long> keySerde =
        Serdes.Long(); // same as // Serdes.LongSerde keySerde=new Serdes.LongSerde();
    ...
    KStream<Long, Order> paymentStream =
        builder.stream(
            ...
            Consumed.with(
                keySerde,
                valueSerde)); // Consumed With == passing some parameters for configuring the
    // generated stream

    KStream<Long, Order> stockStream = builder.stream(STOCK, Consumed.with(keySerde, valueSerde));
    ...
    paymentStream
        .join(
            stockStream,
            orderService
                ::confirm, // the value joiner == the one responsible for joining the two records
            JoinWindows.of(
                Duration.ofSeconds(
                    10)), // timestamps of matched records must fall within this window of time
            StreamJoined.with(
                keySerde,
                valueSerde,
                valueSerde) // the key must be the same, 1st stream serde, 2nd stream serde
            )
  ...
  /** To build a persistent key-value store This KTable will be used to store all the Orders * */
  @Bean
  public KTable<Long, Order> table(StreamsBuilder builder) {
    ...
    KStream<Long, Order> stream =
        builder.stream(ORDERS, Consumed.with(keySerde, valueSerde))
            .peek((k, v) -> log.info("Kafka persistence table: key[{}],value[{}]", k, v));

    return stream.toTable(
        Materialized.<Long, Order>as(store).withKeySerde(keySerde).withValueSerde(valueSerde));
  }
}
